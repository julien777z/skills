...
- On host macOS 26.6.2 (25G83), Studio 0.737.0.7371584 rendered the serialized three-outfit asset cleanly from front, side, and rear, including animated ViewportFrame previews.
- Guest macOS 15.7.7 (24G720) reproduced torn geometry in a minimal scene without game scripts using the same signed Studio 0.737.0.7371584 bundle. This controls the Studio-version difference.
- Guest Studio 0.739.0.7390687 and 0.740.0.7400927 also showed corruption in Workspace and ViewportFrames, including near the origin with the published player closed. The orientation gizmo distorted too.
- The guest used Apple Paravirtual Metal and Tart 2.37.0. Cold start and eager-rendering changes did not repair meshes; OpenGL startup reported `Error creating pixel format`. Automatic graphics and `EagerBulkExecution=false` were restored.
- The guest OS updater reported `Failed to find SFR recovery volume` (SUOSU 201 / SUMacController 7723 / MobileSoftwareUpdate 1256). The base-image disk lacks the separate recovery partition.
...
Supporting sources: the `roblox-studio` skill's [VM access](../../.agents/skills/roblox/roblox-studio/references/isolated-testing.md) and [hidden viewer](../../.agents/skills/roblox/roblox-studio/references/hidden-vm-viewer.md) references cover runner ownership, preservation, and capture limits. The [Tart recovery-partition explanation](https://github.com/openai/tart/issues/1232#issuecomment-4449144329) describes the base-image update prerequisite. The [Tart graphics compatibility discussion](https://github.com/openai/tart/issues/1032) notes dependence on guest and host OS versions; it does not diagnose this defect.
