...
| [`cr`](.agents/skills/git/cr/SKILL.md) | Triage and resolve the pull request's open review threads, run multi-subagent code-simplify across the complete pull request and related code, then run the high-effort fix review, repair failed checks, squash-merge, and verify the pull request is merged. |
...
| [`defer-scope`](.agents/skills/deferrals/defer-scope/SKILL.md) | Record deferred work in the repository it affects, using Linear or that repository's ledger as an availability fallback; with no scope, read its active records. |
...
| [`test-skill`](.agents/skills/authoring/test-skill/SKILL.md) | Prove a skill edit changes what a reader does: rebuild the miss that prompted it, run reviewers on the edited and the original text, and score both against stated criteria before the pull request merges. |
