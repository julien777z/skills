...
6. In create/audit/extend design-only requests, return the proposed canon and implementation plan. When implementation is authorized, apply the selected improvements through the game's existing content owners and applicable implementation skills. Keep rewards, quest gates, UI instructions, destinations, environmental states, and recap text consistent. Verify the actual first launch, onboarding, playable sequence, ending, and revisits. Record proposed, implemented, and playtested status per beat or feature with evidence; prose and generated art do not prove playable implementation. Update durable canon with appearances, relationships, learned concepts, setups/payoffs, and preserved victories so future audits can read the full story.
