...
1. Establish mode: **create** for a new story, **audit** for an existing story, **extend** for a completed arc receiving new scope. Read the durable canon and actual first-launch flow, onboarding, quests, dialogue, character visuals, world layout, progression, ending, aftermath, and revisits. Follow the complete player journey before deciding what to preserve or revise; a late-game quest list or previous synopsis is insufficient. Record the audience, tone, player role, gameplay loop, scope, constraints, existing canon, and known implementation gaps. State reasonable assumptions; ask only for decisions that materially block the work.
2. Read the [story criteria](references/story-criteria.md) and [mode criteria](references/modes.md). Define what success means for this game before comparing proposals. Include character staging: where and why enemies and NPCs are present, what they are doing, when they become visible or interactive, and how the player meets them. In audit mode, identify concrete weak beats and contradictions with source pointers, then preserve what already works.
...
5. Polish a complete story web and implementation map. Expand regions, routes, worlds, or maps when they earn their production cost through distinct playable beats. Propose major characters’ appearance and motion and compare the cast together: identify how each is recognizable without a nameplate, including non-color cues, and specify variants where shared base assets make different characters look alike. Existing rigs are a baseline to evaluate, not an appearance proposal. Then identify required environment, art, systems, dialogue, and progression changes, separate essential scope from optional scope, and preserve explicit user limits. A scope proposal is not proof that a map exists. Separate major worlds into the project's established place/level boundaries.
6. In create/audit/extend design-only requests, return the proposed canon and implementation plan. When implementation is authorized, apply the selected improvements through the game's existing content owners and applicable implementation skills. Keep rewards, quest gates, UI instructions, destinations, environmental states, and recap text consistent. Verify the actual first launch, onboarding, playable sequence, ending, and revisits, including character arrivals, activities, interactions, and departures from the player's normal approach. Record proposed, implemented, and playtested status per beat or feature with evidence; prose and generated art do not prove playable implementation. Update durable canon with appearances, staging, relationships, learned concepts, setups/payoffs, and preserved victories so future audits can read the full story.
