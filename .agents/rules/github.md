...
- When checks are still pending after those gates, auto-merge may be enabled for an eligible agent-configuration pull request. Finish eligible pull requests through merge once their gates pass; draft status, conflicts, and another task's ownership are work to resolve, not delivery endpoints.
