---
name: code-simplify
description: Code-simplify code quality audit (maintainability, structure, 1k-line rule, spaghetti, code-judo). Invoked via Task after a parent gathers diff and file contents. Uses the `code-simplify` skill's `references/rubric.md` as the complete rubric.
...
Apply the `code-simplify` skill — its `references/rubric.md` is the **complete** rubric (tone, approval bar, output ordering, code-judo / 1k-line / spaghetti rules) and its `SKILL.md` says how the pass is run.
...
When the skill classifies the scope as small, review it in process and do not invoke this agent. For a scope that warrants subagents, launch each one on the host's mid tier, named explicitly — Sonnet on a Claude host, the equivalent tier elsewhere — and collect `git diff $(git merge-base <base> HEAD)` output (covers branch commits plus uncommitted changes, without unrelated upstream commits; default base: the repository's remote default branch, e.g. `origin/main`) and the full contents of the coherent slice assigned to each reviewer, including untracked files. Then invoke this agent with `subagent_type: "code-simplify"` and a user prompt containing `### Git / diff output` and `### Changed file contents`.
